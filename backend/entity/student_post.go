...
type StudentPost struct {
	...
	// ✅ 1. แก้ไขตรงนี้: ให้ GORM จัดการ Foreign Key เองโดยอัตโนมัติ
	EmploymentTypeID *uint           `json:"employment_type_id"`
	EmploymentType   *EmploymentType `json:"employment_type"` // <--- **ลบ gorm:"foreignKey:..." ออก**

	// 2. ความสัมพันธ์กับ Student (เจ้าของโพสต์)
	StudentID *uint   `json:"student_id" gorm:"not null"`
	Student   Student `json:"student" gorm:"foreignKey:StudentID;references:ID"`

	// 3. ความสัมพันธ์กับไฟล์แนบ
