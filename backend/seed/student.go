...
func SeedStudents(db *gorm.DB) {
    ...
    for i := int(count) + 1; i <= 30; i++ {
        ...
        // 🟢 แก้ไขจุดที่ 1: ดักกรณี Username ซ้ำ ให้ Force Update รหัสผ่านใหม่ทับทันที
        var user entity.User
        var existingUser entity.User
        if err := db.Where("username = ?", username).First(&existingUser).Error; err == nil {
            // ถ้าเจอ User เดิมค้างอยู่ บังคับเขียนรหัสผ่านใหม่ทับไปเลย
            db.Model(&existingUser).Update("password", string(password))
            user = existingUser
        } else {
            // ถ้าไม่มีอยู่เลย สร้างใหม่ตามปกติ
            user = entity.User{
                Username: username,
        ...
        // 🟢 แก้ไขจุดที่ 2: ป้องกัน Foreign Key ตีกัน (ถ้าตารางแม่โล่ง ให้ Safe ค่าไว้ที่ ID: 1)
        var genderID uint = 1
        if len(genders) > 0 {
        ...
        // ใช้ FirstOrCreate ของ Student ตามปกติ
