...
func SeedDatabase() {
	...
	jobposts := []entity.Jobpost{
		{
			...
			Deadline:          time.Date(2025, 8, 31, 0, 0, 0, 0, time.UTC),
			Status:            entity.Close,
			ImageURL:          nil,
		...
		},
		{
			...
			LocationJob:       "-",
			EmployerID:        employer.ID,
			JobCategoryID:     jobCategories[12].ID,
			EmploymentTypeID:  employmentTypes[3].ID,
			SalaryTypeID:      salaryTypes[3].ID,
		},
	}
	...
	jobApplications := []entity.JobApplication{
		...
		},
		{
			...
			LastUpdate:        time.Now().AddDate(0, 0, -7),
		...
		{
			ApplicationStatus: entity.StatusAccepted,
			LastUpdate:        time.Now().AddDate(0, 0, -3),
			ApplicationReason: "ได้รับการคัดเลือก",
			StudentID:         student.ID,
